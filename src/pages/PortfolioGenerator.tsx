...
const PortfolioGenerator = () => {
  ...
  // Safely process data with robust error handling
  const processedData = useMemo(() => {
    if (!rawData || !Array.isArray(rawData)) return [];
    
    return rawData.filter(option => {
      ...
      try {
        ...
        const strikePrice = Number(option.StrikePrice) || 0;
        const bid = Number(option.Bid) || 0;
        const ask = Number(option.Ask) || bid;
        const underlyingValue = 100000; // Default value
        
        const numberOfContracts = strikePrice > 0 ? Math.round((underlyingValue / strikePrice) / 100) : 0;
        const bidAskMidPrice = (bid + ask) / 2;
        ...
        console.warn('Error processing option:', option, err);
        return null;
      }
  ...
  const generatePortfolio = () => {
    ...
    let filteredOptions = [...processedData];
    ...
    if (strikeBelowPeriod !== null) {
      filteredOptions = filteredOptions.filter(option => {
        try {
          ...
          console.warn('Error filtering option:', option, err);
          return false;
        }
      });
    }
    ...
    if (minProbabilityOfWorthless !== null) {
      filteredOptions = filteredOptions.filter(option => {
        try {
          ...
          console.warn('Error checking probability:', option, err);
          return false;
        }
      });
    }
    ...
    const recalculatedOptions = filteredOptions.map(option => {
      try {
        ...
        console.warn('Error recalculating option:', option, err);
        return null;
      }
    }).filter(Boolean);

    // Group by stock and select best option per stock
    ...
    optionsByStock.forEach((options, stockName) => {
      try {
        ...
        console.warn('Error selecting best option for stock:', stockName, err);
      }
    });
  ...
  const uniqueExpiryDates = useMemo(() => {
    try {
      ...
      return [...new Set(processedData.map(option => option.ExpiryDate).filter(Boolean))].sort();
    } catch (err) {
      console.warn('Error getting expiry dates:', err);
      return [];
    }
  }, [processedData]);
  ...
  if (!processedData || processedData.length === 0) {
    return (
      <div className="container mx-auto p-6">
        ...
        <div className="text-center mt-8">No valid options data available</div>
      </div>
    );
  }
